# model raw response - run 006 direct_answer

下面给出一个可运行的 shrink-only Seam Carving 基线实现，以及协议化实验脚本。
...
def main():
    ...
    out_root = root / 'outputs' / 'hw1_op1' / 'run_006_direct_answer'
    ...
    for image_name, case_name in cases:
        ...
        print(f'[{image_name}] seam_width {time.perf_counter()-t0:.3f}s')
        io.imsave(str(case_dir / 'seam_width.png'), seam_width)

        t0 = time.perf_counter()
        seam_height = seam_carve_image(im, (target_h, w))
        print(f'[{image_name}] seam_height {time.perf_counter()-t0:.3f}s')
